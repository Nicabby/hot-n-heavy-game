...
export default function Lobby() {
  ...
  // Remove unused goToStep function for now
  ...
  const handleCreateGame = async () => {
    try {
      ...
      if (result.success) {
        ...
        // Store game setup in localStorage for the game session
        localStorage.setItem('currentGame', JSON.stringify({
          ...gameSetup,
          gameId: result.gameId
        }))
        
        // Navigate to consent or game page
                  ...
                  <div key={index} className="flex gap-2 items-center">
                    ...
                    {gameSetup.participants.length > 1 && (
                      <button
                        ...
                        className="px-3 py-3 rounded-lg text-white font-bold hover:opacity-80"
                    ...
                    {[
                      ...
                      <label key={option.key} className="flex items-center space-x-3 cursor-pointer p-2 rounded-lg hover:bg-gray-50">
                        <input
                          ...
                          style={{accentColor: '#bb3309'}}
                        />
                        <span style={{color: '#5b5450'}}>{option.label}</span>
                      </label>
                    ))}
                  </div>
                </div>
